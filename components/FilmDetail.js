...
class FilmDetail extends React.Component {
  ...
  static navigationOptions = ({ navigation }) => {
    const { params } = navigation.state
      if (params.film != undefined && Platform.OS === 'ios'){
        return {
          headerRight: <TouchableOpacity style={styles.share_touchable_headerrightbutton}
                          onPress={() => params.shareFilm()}>
                            <Image style={styles.share_image} source={require('../Image/ic_share.ios.png')} />
                       </TouchableOpacity>
      }
    }
  }
  ...
  componentDidMount() {
    ...
    if (favoriteFilmIndex !== -1) { // Film déjà dans nos favoris, on a déjà son détail... Pas besoin d'appeler l'API ici, on ajoute le détail stocké dans notre state global au state de notre component
      this.setState({
    ...
    if (film != undefined) {
      return (
        <ScrollView style={styles.scrollview_container}>
          ...
          <Text style={styles.default_text}>Bénéfice : {numeral(film.revenue).format('0,0[.]00 $')}</Text>
